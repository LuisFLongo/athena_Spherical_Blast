...
void EquationOfState::ConservedToPrimitive(
  ...
  printf("Entering ConToPrim \n");
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        if (std::isfinite(prim(IDN,k,j,i))!=1){
		printf("BC was imposed before this step \n");
	}
...
void EquationOfState::ApplyPrimitiveFloors(AthenaArray<Real> &prim, int k, int j, int i) {
  printf("ApplyPrimitiveFloors \n");
  if (std::isfinite(prim(IDN,i))!=1){
         printf("BC was imposed before this step \n");
  }
  Real& w_d  = prim(IDN,i);
  Real& w_p  = prim(IPR,i);
...
void EquationOfState::ApplyPrimitiveConservedFloors(
    ...
    int k, int j, int i) {  printf("ApplyPrimitiveConservedFloors \n");
  if (std::isfinite(prim(IDN,k,j,i))!=1){
 	 printf("BC was imposed before this step \n");
  }
  Real gm1 = GetGamma() - 1.0;
