...
//
	...
	//float Nthetadown = ( BCThetamin -  (-M_PI/2) )/BCDtheta;
	//float Nthetaup   = (  3*M_PI/2  - BCThetamax )/BCDtheta;

        //float Nphidown   = ( BCPhimin - (-M_PI/2) )/BCDphi;
        //float Nphiup     = ( 5*M_PI/2 -  BCPhimax )/BCDphi;
 
	//int   BCNGhost = std::ceil(std::max(std::max(Nthetadown, Nthetaup),std::max(Nphidown, Nphiup)));
	
	int BCNGhost = BCNghts; 
	
	table->SetSize(5,BCNtheta+2*BCNGhost,BCNphi+2*BCNGhost,BCNt);
	
        printf("Test Reading outflow 0.1\n");
	
	table -> SetY3lim(BCThetamin - BCNGhost*BCDtheta ,BCThetamax + BCNGhost*BCDtheta);
        table -> SetY2lim(BCPhimin   - BCNGhost*BCDphi   ,BCPhimax   + BCNGhost*BCDphi  );
        table -> SetY1lim(0                              ,BCTmax     - BCTmin           );
	//table -> data;	

	printf("thetamin = %6.40lf \n",BCThetamin - BCNGhost*BCDtheta );
	printf("thetamax = %6.40lf \n",BCThetamax + BCNGhost*BCDtheta );
	
	printf("phimin = %6.40lf \n", BCPhimin    - BCNGhost*BCDphi);
        printf("phimax = %6.40lf \n", BCPhimax    + BCNGhost*BCDphi);
	...
	while (!fin.eof()){
		...
		float BCpress     = Token[17]; // extra - pressure coming from interpolation
	...
	//	printf("BCmphi   = %6.4lf\n", BCmphi);
		...
		//int BCntheta = BCmtheta + BCNGhost;
		//int BCnphi   = BCmphi   + BCNGhost;
		//int BCnt     = BCmt               ;

		if (BCntheta > BCNtheta+2*BCNGhost or BCntheta <0) {
			...
			break;
                ...
                if (BCnphi > BCNphi+2*BCNGhost or BCnphi < 0) {
		...
		table->data(1 , BCntheta, BCnphi, BCnt ) = Kval * std::pow(BCrho,Gammaval); //pressure - here I am assuming polytropic EOS
                //table->data(1 , BCntheta, BCnphi, BCnt ) = BCpress; // pressure - here I am reading from the table
	...
	for (int kk = 0 ; kk <= BCNt ; kk++) {
		for (int ii =0 ; ii<= BCNGhost-1 ; ii++){
			for (int jj =0; jj <= BCNGhost-1 ; jj++){
				...
				table->data(0 , ii, jj, kk ) = table->data(0 , BCNtheta+BCNGhost-ii, BCNphi+BCNGhost-jj, kk ); //density 
                		table->data(1 , ii, jj, kk ) = table->data(1 , BCNtheta+BCNGhost-ii, BCNphi+BCNGhost-jj, kk ); //pressure
				table->data(2 , ii, jj, kk ) = table->data(2 , BCNtheta+BCNGhost-ii, BCNphi+BCNGhost-jj, kk ); //vr
				table->data(3 , ii, jj, kk ) = table->data(3 , BCNtheta+BCNGhost-ii, BCNphi+BCNGhost-jj, kk ); //vt
				table->data(4 , ii, jj, kk ) = table->data(4 , BCNtheta+BCNGhost-ii, BCNphi+BCNGhost-jj, kk ); //vp
				//printf("AA \n");

                        	table->data(0 , ii, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(0 , BCNtheta+BCNGhost-ii, BCNGhost+jj, kk ); //density 
                        	table->data(1 , ii, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(1 , BCNtheta+BCNGhost-ii, BCNGhost+jj, kk ); //pressure
                        	table->data(2 , ii, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(2 , BCNtheta+BCNGhost-ii, BCNGhost+jj, kk ); //vr
                        	table->data(3 , ii, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(3 , BCNtheta+BCNGhost-ii, BCNGhost+jj, kk ); //vt
                        	table->data(4 , ii, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(4 , BCNtheta+BCNGhost-ii, BCNGhost+jj, kk ); //vp
				//printf("BB \n");

                        	table->data(0 , BCNtheta+2*BCNGhost-ii-1, jj, kk ) = table->data(0 , BCNGhost+ii, BCNphi+BCNGhost-jj, kk ); //density 
                        	table->data(1 , BCNtheta+2*BCNGhost-ii-1, jj, kk ) = table->data(1 , BCNGhost+ii, BCNphi+BCNGhost-jj, kk ); //pressure
                        	table->data(2 , BCNtheta+2*BCNGhost-ii-1, jj, kk ) = table->data(2 , BCNGhost+ii, BCNphi+BCNGhost-jj, kk ); //vr
                        	table->data(3 , BCNtheta+2*BCNGhost-ii-1, jj, kk ) = table->data(3 , BCNGhost+ii, BCNphi+BCNGhost-jj, kk ); //vt
                        	table->data(4 , BCNtheta+2*BCNGhost-ii-1, jj, kk ) = table->data(4 , BCNGhost+ii, BCNphi+BCNGhost-jj, kk ); //vp
				//printf("CC \n");

                        	table->data(0 , BCNtheta+2*BCNGhost-ii-1, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(0 , BCNGhost+ii, BCNGhost+jj, kk ); //density 
                        	table->data(1 , BCNtheta+2*BCNGhost-ii-1, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(1 , BCNGhost+ii, BCNGhost+jj, kk ); //pressure
                        	table->data(2 , BCNtheta+2*BCNGhost-ii-1, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(2 , BCNGhost+ii, BCNGhost+jj, kk ); //vr
                        	table->data(3 , BCNtheta+2*BCNGhost-ii-1, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(3 , BCNGhost+ii, BCNGhost+jj, kk ); //vt
                        	table->data(4 , BCNtheta+2*BCNGhost-ii-1, BCNphi+2*BCNGhost-jj-1, kk ) = table->data(4 , BCNGhost+ii, BCNGhost+jj, kk ); //vp
				//printf("DD \n");
			}
		}
	}
