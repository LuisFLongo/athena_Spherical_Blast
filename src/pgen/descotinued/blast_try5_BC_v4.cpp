...
// Comment : the version of this pgen is the same as the one of same name in master1_7_1 
//           nevertheless some quantities have a slightly different name between master and princeton
//           the only difference between these files is the chance pblock ->pmb in some parts
//           I left these parts comment out and one can easily find them and change if necessary
...
Real threshold, method, rbc, velfloor, BCThetamin, BCPhimin, BCTmin, BCThetamax, BCPhimax, BCTmax, BCMtheta, BCMphi,BCMt, BCDtheta, BCDphi, BCDt, Gammaval, Kval;

int BCNtheta, BCNphi,BCNt;
...
//
	...
	table->SetSize(5,BCNtheta,BCNphi,BCNt);
	
        //printf("Test Reading outflow 0.1\n");
	
	table -> SetY3lim(BCThetamin ,BCThetamax ) ;
        table -> SetY2lim(BCPhimin   ,BCPhimax   ) ;
	...
	while (!fin.eof()){
		...
		//printf("Test Reading Outflow 3 \n");
	        ...
	        float BCphi    = sign(BCy) * std::acos(BCx /std::sqrt( std::pow(BCx ,2) + std::pow(BCy , 2)))         ;
...
void InnerBoundary(MeshBlock *pmb, Coordinates *pcoord,
	...
	for (int k=kl; k<=ku; ++k) {
		//printf("Inside the BC loop \n");
				...
				//printf("theta = %6.4lf \n", thval);
				//printf("phival = %6.4lf \n", phival);
                            	//printf("inside BC loop \n");
				//printf("Testing bc 1 \n");
				//prim(IDN, k, j, il-i) = 1.0;
				prim(IDN, k ,j ,il-i) = DenProfile(thval, phival, time);
				//printf("density = %6.40lf \n", prim(IDN, k, j, il-i));
				//prim(IPR, k, j, il-i) = 0.5;
                                prim(IPR, k, j, il-i) = PressProfile(thval, phival, time);
				//printf("pressure = %6.40lf \n", prim(IPR, k, j, il-i));
                               	//prim(IVX, k, j, il-i) = 0.80; // rval * std::sin(thval) * std::cos(phival) ;
                                prim(IVX, k, j, il-i) = VRProfile(thval, phival, time);
				//printf("vr = %6.40lf \n", prim(IVX, k, j, il-i));
                               	//prim(IVY, k, j, il-i) = 0.50; //rval * std::sin(thval) * std::sin(phival) ;
                                prim(IVY, k, j, il-i) = VTProfile(thval, phival, time);
				//printf("vtheta = %6.40lf \n", prim(IVY, k, j, il-i));
                               	//prim(IVZ, k, j, il-i) = 0.01;//rval * std::cos(thval) * std::pow(std::sin(thval),2);
                                prim(IVZ, k, j, il-i) = VPProfile(thval, phival, time);
				//printf("vphi = %6.40lf \n", prim(IVZ, k, j, il-i));
				//printf("Testing bc 2 \n");
				//printf("####################################################################### \n");
                       	}
