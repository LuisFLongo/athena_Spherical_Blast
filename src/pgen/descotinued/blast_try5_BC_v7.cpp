...
Real threshold, method, rbc, velfloor, BCThetamin, BCPhimin, BCTmin, BCThetamax, BCPhimax, BCTmax, BCMtheta, BCMphi,BCMt , BCtin , BCDtheta, BCDphi, BCDt, Gammaval, Kval; //, BCMghts
...
//
	...
	//int   BCNghts = std::ceil(std::max(std::max(Nthetadown, Nthetaup),std::max(Nphidown, Nphiup)));
	...
	//	printf("BCmphi   = %6.4lf\n", BCmphi);
		...
		//if (BCntheta > BCNtheta+2*BCNghts or BCntheta < 0) {
                if (BCntheta > BCNtheta+BCNghts or BCntheta < BCNghts) {
			...
			return;
                ...
                if (BCnphi > BCNphi+BCNghts or BCnphi < BCNghts) {
		...
		float Trhomin = 9.99908793 * std::pow(10,-15);
		float Trhomax = 2.04474417 * std::pow(10,-10);

		float Tpressmin = 8.254368620859124 * std::pow(10,-20);
		float Tpressmax = 1.952452818229532 * std::pow(10,-14);

		float Tvrmin = -0.0037080970266283662;
		float Tvrmax =  0.5145687580274914;

		float Tvtmin = -0.00025542644782544616;
		float Tvtmax =  0.00025542644782544616;

		float Tvpmin = -0.004348400171912313;
		float Tvpmax =  0.002945209113444522;
