...
Expansion::Expansion(MeshBlock *pmb, ParameterInput *pin) {
  ...
  if (pmb->precon->xorder < 4) {reconCoeff = false;}
  else { reconCoeff = true;}
...
void Expansion::ExpansionSourceTerms(const Real dt, const AthenaArray<Real> *flux, const AthenaArray<Real> &prim, AthenaArray<Real> &cons) {
  ...
  AthenaArray<Real> &v2f = vf[X2DIR];
  AthenaArray<Real> &v3f = vf[X3DIR];
  //std::cout << "Started expSrc" << std::endl;
  for (int k = ks; k<=ke;++k) {
    for (int j = js; j<=je;++j) {
#pragma omp simd
      for (int i = is; i<=ie;++i) {
  ...
  //std::cout << "Ended expSrc" << std::endl;
...
void Expansion::GridEdit(MeshBlock *pmb,bool lastStage){
  //FACE CENTERED
  //x1
  //std::cout << "Started expEdit" << std::endl;
  if (x1Move) {
    for (int i=il; i<=iu+1; ++i){
  ...
  //std::cout << "Ended move faces" << std::endl;
  ...
  if (reconCoeff) {
    ...
    //std::cout << "Ended Reconstruction faces" << std::endl;
    ...
    //std::cout << "Ended Curvilinear" << std::endl;
  ...
  if (COORDINATE_SYSTEM == "cartesian") {
    ...
    //std::cout << "Ended Volume Base" << std::endl;
...
#pragma omp simd
  ...
  //std::cout << "Ended expEdit" << std::endl;
  return;
}
...
Real Expansion::GridTimeStep(MeshBlock *pmb){
  ...
  Real mydt = 10.0* pmesh->dt;

  Real min_dt = (FLT_MAX);
  Real dtEx = 10.0*min_dt;
  int count = 0;
  Real overStep = 0.0;
  if (x1Move) {
    for (int i = il; i<=iu+1;i++){
