...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  printf("Entering InitUserMeshData \n");
  ...
  printf("Exiting InitUserMeshData \n");
  return;
}
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  printf("Entering ProblemGenerator \n");
  ...
  printf("Exiting ProblemGenerator \n");
...
void DMRInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
                Real time, Real dt,
                int il, int iu, int jl, int ju, int kl, int ku, int ngh) {
  printf("Entering DMRInnerx1 Condition \n");
  ...
  for (int j=jl; j<=ju; ++j) {
    ...
    for (int i=1;  i<=ngh; ++i) {
      printf("i=%d \n",i);
      int size0 = prim.GetDim1();
      printf("Size Dim 0 = %d \n", size0);
      int size1 = prim.GetDim2();
      printf("Size Dim 1 = %d \n", size1);
      int size2 = prim.GetDim3();
      printf("Size Dim 2 = %d \n", size2);
      int size3 = prim.GetDim4();
      printf("Size Dim 3 = %d \n", size3);      
  ...
  printf("Exiting DMRInnerx1 Condition \n");
...
void DMRInnerX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
                Real time, Real dt,
                int il, int iu, int jl, int ju, int kl, int ku, int ngh) {
  printf("Entering DMRInnerx2 Condition \n");
  Real d0 = 8.0;
  Real e0 = 291.25;
  Real u0 =  8.25*std::sqrt(3.0)/2.0;
  Real v0 = -8.25*0.5;
  ...
  printf("Exiting DMRInnerx2 Condition \n");
...
void DMROuterX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim, FaceField &b,
                Real time, Real dt,
                int il, int iu, int jl, int ju, int kl, int ku, int ngh) {
  printf("Entering DMROuterx2 Condition \n");
  ...
  printf("Exiting DMROuterx2 Condition \n");
...
int RefinementCondition(MeshBlock *pmb) {
  printf("Entering Refinement Condition \n");
  ...
  // otherwise, stayi
  printf("Exeting Refinement Condition \n");
  return 0;
}
