...
// Comment : this version seats on top of gr_blast_try5_BC_v9 
// 	     the only difference so far from the previous version is some corrections
// 	     in the BC implementation of the interpolated data.
// 	     the error of not evolving the ghost zones persists.
// 	     this version still uses the angular coordinate velocity.
// 	     for the implementation of the angular orthonormal velocity, look at the blast_try5_BC_v10. 	
// 	     in order to surpass the problem with the blast_try5_BC_v9 code not
// 	     working with the -s (special relativity) flag.
// 	     in this version I am extending the problem to the GR case 
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  printf("Entering Problem Generator\n");
 ...
 // }
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=il; i<=iu; ++i) {
        ...
        //if (rval < 600 ){
        //	phydro->w(IDN,k,j,i) = phydro->w1(IDN,k,j,i) = 50000.0*da;
        //	phydro->w(IPR,k,j,i) = phydro->w1(IPR,k,j,i) = 50000.0*pa;
	//}
        //else{
  ...
  printf("Exeting Problem Generator\n");
  return;
}
