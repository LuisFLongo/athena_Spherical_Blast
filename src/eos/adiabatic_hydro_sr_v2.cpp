...
void EquationOfState::ConservedToPrimitive(
  ...
  printf("Enter ConservedToPrimitive \n");
  // Parameters
  ...
  for (int k=kl; k<=ku; ++k) {
    for (int j=jl; j<=ju; ++j) {
      ...
      for (int i=il; i<=iu; ++i) {
        ...
        if (std::isfinite(prim(IDN,k,j,i))!=1){
		Testing = true;
                //printf("BC was imposed before this step : ConservedToPrimitive \n");
        }
	printf("den = %6.40lf \n",prim(IDN,k,j,i) );
	...
	//  printf(" Adjust conserved density and energy \n");
	...
	//  printf("Fail! 3 \n");
	  ...
	  //printf("den = %6.40lf \n =", prim(IDN,k,j,i));
        ...
        if (fixed) {
	  //printf("Fixed! Ensure conserved variables match primitives \n");
...
void EquationOfState::PrimitiveToConserved(
  ...
  // Calculate reduced ratio of specific heatsi
  printf("Enter PrimitivetoConserved \n");
