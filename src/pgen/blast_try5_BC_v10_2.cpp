...
// Comment : this version seats on top of blast_try5_BC_v10_2 but
// 	     commenting out the inclusion of internal energy via the BC function
...
//
		...
		float BCtheta  = std::max(std::min(BCtheta1,BCThetamax),BCThetamin);
                ...
                float BCvr = ( BCx * BCvx + BCy * BCvy  + BCz * BCvz  ) / std::sqrt( BCx*BCx + BCy*BCy + BCz*BCz ) ;
              	float BCvt = ( BCx * BCz * BCvx + BCy * BCz * BCvy - ( BCx*BCx + BCy*BCy ) * BCvz) / ( std::sqrt( BCx*BCx +BCy*BCy ) * std::sqrt( BCx*BCx + BCy*BCy + BCz*BCz ) ) ;
              	float BCvp = (- BCy * BCvx + BCx * BCvy )/std::sqrt(BCx*BCx + BCy*BCy);
