...
void InflowBoundary(MeshBlock *pmb, Coordinates *pcoord, AthenaArray<Real> &prim,
    ...
    int ngh) {
  // Set hydro variables
  printf("Entering InflowBoundary function\n");
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      for (int i = il - ngh; i <= il - 1; ++i) {
        prim(IDN,k,j,i) = std::sqrt(-5.5) ;//prim(IDN,k,j,il);
        prim(IPR,k,j,i) = std::sqrt(-2.2) ;//prim(IPR,k,j,il);
        prim(IVX,k,j,i) = std::sqrt(-0.8) ;//std::min(prim(IVX,k,j,il), static_cast<Real>(0.0));
        prim(IVY,k,j,i) = 0.2 ;//prim(IVY,k,j,il);
        prim(IVZ,k,j,i) = 0.3 ;//prim(IVZ,k,j,il);
      }
    }
  }
  if (!MAGNETIC_FIELDS_ENABLED) {
