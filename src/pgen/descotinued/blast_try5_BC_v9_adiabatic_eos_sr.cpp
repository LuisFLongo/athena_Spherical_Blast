...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k=ks; k<=ke; k++) {
	...
	phydro->u(IVX,k,j,i) = 0.000001;
        phydro->u(IVY,k,j,i) = 0.000001;
        phydro->u(IVZ,k,j,i) = 0.000001;
        //phydro->u(IEN,k,j,i) = pres/gm1;
        //if (RELATIVISTIC_DYNAMICS){  // this should only ever be SR with this file
        //  phydro->u(IEN,k,j,i) += den;
	//}
        //set-up of conservative variable by following the src/eos/adiabatic_hydro_sr.cpp as template
  	Real gamma_prime = gamma/(gamma-1.0);
	Real u1 = phydro->u(IVX,k,j,i);
        Real u2 = phydro->u(IVY,k,j,i);
        Real u3 = phydro->u(IVZ,k,j,i);

        Real u0 = std::sqrt(1.0 + SQR(u1) + SQR(u2) + SQR(u3) );

        // Set conserved quantities
        Real wgas_u0 = (da + gamma_prime * pa) * u0;
        Real d = da * u0;
        Real e = wgas_u0 * u0 - pa;
        Real m1 = wgas_u0 * u1;
        Real m2 = wgas_u0 * u2;
        Real m3 = wgas_u0 * u3;
        phydro->w(IDN,k,j,i) = d;
        phydro->w(IEN,k,j,i) = e;
        phydro->w(IM1,k,j,i) = m1;
        phydro->w(IM2,k,j,i) = m2;
        phydro->w(IM3,k,j,i) = m3;
