...
//           introducing Ye as an extra input in the BC and also 
//           changing the problem generator to include Ye in the
//           initial atmosphere
...
Real threshold, method, da, pa, Yea;
...
Real YeProfile(const Real th, const Real ph, const Real tm);
// LFLM inclusion ends

void Mesh::InitUserMeshData(ParameterInput *pin) {
//LFLM inclusion start
  //printf("entering  Mesh::InitUserMeshData \n  ");
  pa   = pin->GetOrAddReal("problem", "pamb", 1.0);
  da   = pin->GetOrAddReal("problem", "damb", 1.0);
  Yea  = pin->GetOrAddReal("problem", "Yeamb",1.0);
...
//        pscalars->s(0,k,j,i) = Yeval;
//        printf("den = %6.40lf \n", phydro->u(IDN,k,j,i) ); 
//	printf("Ye amb = %6.4lf \n",Yeval );
 
        constexpr int scalar_norm = NSCALARS > 0 ? NSCALARS : 1.0;

//        printf("scalar_norm = %6.40lf \n",scalar_norm );

        if (NSCALARS > 0) {
          for (int n=0; n<NSCALARS; ++n) {
	    //printf("testing 1 \n");

            pscalars->s(n,k,j,i) = Yeval*den ;

	    //printf("testing 2 \n");
//	    pscalars->r(n,k,j,i) = Yeval ;
//	    printf("scalar = %6.40lf \n",pscalars->s(n,k,j,i));
          }
        }
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    printf("NOT TESTED FOR MAGNETIC FIELDS PROCEEED WITH CAUTION");
    for (int k=ks; k<=ke; ++k) {
      for (int j=js; j<=je; ++j) {
...
//
        ...
        table->SetSize(9,BCNtheta+2*BCNghts,BCNphi+2*BCNghts,BCNt);
	...
	printf("Filling ghost zones \n ");
        ...
        // peridiodical BC on phi
                for (int ii =0 ; ii<=BCNtheta ; ii++){
                        for (int jj =0; jj <=BCNghts ; jj++){
                                ...
                                table->data(7 , BCNghts + ii, jj, kk ) = table->data(7 , BCNghts + ii, BCNphi + jj, kk ); //Ye
                                ...
                                table->data(7 , BCNghts + ii, BCNphi + BCNghts + jj, kk ) = table->data(7 , BCNghts + ii, BCNghts + jj, kk ); //Ye
        ...
        // parity BC on theta as in Table I of https://arxiv.org/pdf/2002.06225.pdf
				...
				table->data(0 , ii, BCNghts + jj, kk ) =   table->data(0 , BCNtheta + ii, BCNghts + jj, kk ); //density
                                ...
                                table->data(7 , ii, BCNghts + jj, kk ) =   table->data(7 , BCNtheta + ii, BCNghts + jj, kk ); //Ye
                                ...
                                table->data(7 , BCNtheta + BCNghts + ii, BCNghts + jj, kk ) =   table->data(7 , BCNghts + ii, BCNghts + jj, kk ); //Ye
        ...
        // filling in the corners of the ghost zones
                for (int ii =0 ; ii<= BCNghts ; ii++){
                        for (int jj =0; jj <= BCNghts ; jj++){
                                ...
                                table->data(7 , BCNtheta + BCNghts + ii , jj , kk ) = table->data(7 , 2 * BCNghts - ii , BCNphi + BCNghts -jj , kk ); //Ye
				...
				// Region 4 to Region 4'
                                ...
                                table->data(7 , BCNtheta + BCNghts + ii, BCNphi + BCNghts + jj, kk ) = table->data(7 , 2 * BCNghts - ii, 2 * BCNghts - jj, kk ); //Ye
                                ...
                                table->data(7 , ii, BCNphi + BCNghts + jj, kk ) = table->data(7 , BCNtheta + BCNghts - ii, 2 * BCNghts - jj, kk ); //Ye
                                ...
                                table->data(7 , ii, jj, kk ) = table->data(7 , BCNtheta + BCNghts - ii, BCNphi + BCNghts - jj, kk ); //Ye
        ...
        for (int kk = 0 ; kk <= BCNt ; kk++) {
              for (int ii =0 ; ii<= BCNtheta+2*BCNghts ; ii++){
                        for (int jj =0; jj <= BCNphi+2*BCNghts ; jj++){
                                if (table->data(0 , ii, jj, kk )==0 and table->data(1 , ii, jj, kk )==0 and table->data(2 , ii, jj, kk )==0 and table->data(3 , ii, jj, kk )==0 and table->data(4 , ii, jj, kk )==0 and table->data(5 , ii, jj, kk )==0 and table->data(6 , ii, jj, kk )==0 and table->data(7, ii, jj, kk )== 0 ) {
                                        NVAL = NVAL+1;
                                        //printf("*********************** \n");
                                        //printf("BCntheta = %d \n", ii);
                                        //printf("BCnphi   = %d \n", jj);
...
Real YeProfile(const Real th, const Real ph, const Real tm) {
  Real Ye;
  Ye = table->interpolate(7, th, ph, tm);
  return Ye ;
}
...
//					}
					...
					if (std::isfinite( YeProfile(thval, phival, timeshift)  ) == 1 ){
						//printf(" BC test Ye \n");
						//prim( 5 , k, j, il-i) = YeProfile(thval, phival, timeshift);
						//printf("s (before) = %6.40lf \n", pmb->pscalars->s(0,k,j,il-i)  );
						//pmb->pscalars->s(0,k,j,il-i) = YeProfile(thval, phival, timeshift)*DenProfile(thval, phival, timeshift) ;					
						//printf("s (after) = %6.40lf \n", pmb->pscalars->s(0,k,j,il-i)  );

                                                //printf("r (before) = %6.40lf \n", pmb->pscalars->r(0,k,j,il-i)  );
                                                pmb->pscalars->r(0,k,j,il-i) = YeProfile(thval, phival, timeshift) ;
                                                //printf("r (after) = %6.40lf \n", pmb->pscalars->r(0,k,j,il-i)  );
